...
export const postOrderDetail = (callThis, event, accountId, productId, productOption) => {
    ...
    if (validator === 'success') {
        ...
        const postData = {
            ...
            orderPaymentOption: callThis.state.paymentOption
        }

        axios.post('http://localhost:5001/api/order/detail/post?account=' + accountId + '&product=' + productId, postData, axiosConfig)
            .then(res => {
                if (res) {
