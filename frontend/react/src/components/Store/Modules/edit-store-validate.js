...
export const editStoreValidate = (callThis) => {   
    ...
    const storeLatitude = callThis.state.storeLatitudeInput;
    const storeLongitude = callThis.state.storeLongitudeInput;

    if(!storeName && !storeEmail && !storePhone && !storeAddress && !storeLatitude && !storeLongitude) {
